...
from .configuration_sasasa2va_chat import SaSaSa2VAChatConfig
...
class SaSaSa2VAChatModel(PreTrainedModel):
    config_class = SaSaSa2VAChatConfig
    ...
    def __init__(self, config: SaSaSa2VAChatConfig, vision_model=None, language_model=None, use_flash_attn=True):
        ...
        if language_model is not None:
            ...
            elif Qwen3ForCausalLM is not None and config.llm_config.architectures[0] == 'Qwen3ForCausalLM':
        ...
        self.grounding_encoder = SAM2(
            ...
            samurai_mode=config.samurai_mode,
            # Hyperparameters for SAMURAI
            stable_frames_threshold=config.stable_frames_threshold,
            stable_ious_threshold=config.stable_ious_threshold,
            min_obj_score_logits=config.min_obj_score_logits,
            kf_score_weight=config.kf_score_weight,
            memory_bank_iou_threshold=config.memory_bank_iou_threshold,
            memory_bank_obj_score_threshold=config.memory_bank_obj_score_threshold,
            memory_bank_kf_score_threshold=config.memory_bank_kf_score_threshold,
        )
        ...
        self.resetted_grounding_encoder = False 
        
    def reset_grounding_encoder(self, samurai_mode: bool=True):
        if not self.resetted_grounding_encoder:
            print(f"Setting mode from \
            {'samurai' if self.config.samurai_mode else 'no samurai'} to \
            {'samurai' if samurai_mode else 'no samurai'}.")
            self.ge_state_dict = self.grounding_encoder.state_dict()
            self.grounding_encoder = SAM2(
                # Whether to use SAMURAI or original SAM 2
                samurai_mode=samurai_mode,
                # Hyperparameters for SAMURAI
                stable_frames_threshold=self.config.stable_frames_threshold,
                stable_ious_threshold=self.config.stable_ious_threshold,
                min_obj_score_logits=self.config.min_obj_score_logits,
                kf_score_weight=self.config.kf_score_weight,
                memory_bank_iou_threshold=self.config.memory_bank_iou_threshold,
                memory_bank_obj_score_threshold=self.config.memory_bank_obj_score_threshold,
                memory_bank_kf_score_threshold=self.config.memory_bank_kf_score_threshold,
            )
            assert self.grounding_encoder.sam2_model.samurai_mode
            self.grounding_encoder.load_state_dict(self.ge_state_dict, strict=True)
            self.eval().cuda()
            self.resetted_grounding_encoder = True
    ...
    def predict_forward(
        ...
        print_answer = False,
        samurai=False,
        selected_fids=None    # Q-frame selected frames
    ):
        if samurai:
            self.reset_grounding_encoder(samurai_mode=samurai)
        ...
        if image is None and video is None and '<image>' not in past_text:
            ...
            if video is not None:
                ...
                assert mode in ['uniform', 'uniform_plus', 'q_frame', 'wrap_around', 'wrap_around_plus'] 
                if mode in ['wrap_around', 'wrap_around_plus']:
                    if len(video) <= 100 or mode == 'wrap_around':
                        selected_frame_indexes = np.arange(100) % len(video)
                        selected_frame_indexes.sort()
                    else:
                        selected_frame_indexes = [int(i * len(video) / 100) for i in range(100)]
                elif 'uniform_plus' == mode:
                    selected_frame_indexes = [int(i * len(video) / 100) for i in range(100)]
                elif 'q_frame' == mode:
                    if selected_fids:
                        assert len(selected_fids) == 100
                        selected_frame_indexes = selected_fids  # pre-sorted
                    else:
                        raise RuntimeError('q_frame mode needs pre-selected frame indexs.')
                else:   # default: uniform
                    selected_frame_indexes = [int(i * len(video) / 100) for i in range(100)]
                for idx in range(0, len(selected_frame_indexes), 10):
                    batch_index = selected_frame_indexes[idx:idx+10]
                    batch = [video[j] for j in batch_index]
                    assert len(batch) == 10

                    first_img = self.transformer(batch[0])
                    pixel_values.append(first_img)

                    width, height = ori_image_size
                    
                    grid = Image.new('RGB', (width * 3, height * 3))
                    for idx, image in enumerate(batch[1:10]):
                        row = idx // 3
                        col = idx % 3
                        grid.paste(image, (col * width, row * height))

                    aggregate_img = self.transformer(grid)
                    pixel_values.append(aggregate_img)
        ...
        if all_seg_hidden_states.shape[0] != 10:
            print("Warning! [SEG] length is not 10: ", all_seg_hidden_states.shape[0])
            if all_seg_hidden_states.shape[0] > 10:
                all_seg_hidden_states = all_seg_hidden_states[:10]
            elif all_seg_hidden_states.shape[0] < 10:
                all_seg_hidden_states = torch.cat([all_seg_hidden_states, all_seg_hidden_states[-1].unsqueeze(0).repeat(10 - all_seg_hidden_states.shape[0], 1)], dim=0)
        g_pixel_values_len = len(input_dict['g_pixel_values'])
        frame2clip, propagate = reverse_index(g_pixel_values_len, mode, selected_frame_indexes=selected_frame_indexes)
        unsequeezed_seg_hidden_states = []
        
        g_pixel_values = input_dict['g_pixel_values']
        if mode in ['uniform_plus', 'q_frame'] and g_pixel_values_len < len(frame2clip):
            ori_frame_len = g_pixel_values_len
            g_pixel_values = [g_pixel_values[i] for i in selected_frame_indexes]    # 凑够100帧
        sam_states = self.grounding_encoder.get_sam2_embeddings(g_pixel_values)
        for idx in range(len(frame2clip)):
            unsequeezed_seg_hidden_states.append(all_seg_hidden_states[frame2clip[idx]].unsqueeze(0))
        if propagate:
            pred_masks = self.grounding_encoder.language_embd_inference(sam_states, unsequeezed_seg_hidden_states)
        else:
            assert len(unsequeezed_seg_hidden_states) == len(g_pixel_values)
            pred_masks = self.grounding_encoder.language_embd_inference_sasasa2va(sam_states, unsequeezed_seg_hidden_states)
        if mode in ['uniform_plus', 'q_frame'] and g_pixel_values_len < len(frame2clip):    # frame2clips最少100个元素
            pred_masks_new = []
            for fid in range(ori_frame_len):
                avg_mask = torch.tensor(selected_frame_indexes) == fid
                pred_masks_new.append(pred_masks[avg_mask].mean(dim=0, keepdim=True))
            pred_masks = torch.cat(pred_masks_new, dim=0)
        w, h = ori_image_size
...
def reverse_index(len_frames, mode, selected_frame_indexes):
    """
    returns: frame2clip idx, whether to propagate
    """
    if len_frames == 100:
        return [i for i in range(10) for _ in range(10)], False
    if mode in ['wrap_around_plus', 'wrap_around']:
        if len_frames < 100:
            selected_frame_indexes = np.arange(100) % len_frames
            selected_frame_indexes.sort()
            frame2clip = []
            for frame_index in range(len_frames):
                positions = np.where(selected_frame_indexes == frame_index)[0]
                first_clip_id = sorted(set(positions // 10))[0]
                frame2clip.append(first_clip_id)
            return frame2clip, False
        else:
            if mode == 'wrap_around_plus':
                return [frame_index * 10 // len_frames for frame_index in range(len_frames)], False
            else: 
                return [i for i in range(10) for _ in range(10)], True
    elif 'uniform_plus' == mode:
        if len_frames < 100: 
            return [i for i in range(10) for _ in range(10)], False
        else: 
