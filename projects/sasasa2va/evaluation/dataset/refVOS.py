...
class RefVOSDataset(BaseEvalDataset):
    def __init__(self,
                 ...
                 expression_file,
                 mask_file,
                 selected_frame_file    # Q-frame selected frame ids
    ):
        super().__init__()
        vid2metaid, metas, mask_dict = self.json_file_preprocess(expression_file, mask_file, selected_frame_file)
    ...
    def json_file_preprocess(self, expression_file, mask_file, selected_frame_file=None):
        with open(expression_file, 'r') as f:
            expression_datas = json.load(f)['videos']
        if selected_frame_file:
            with open(selected_frame_file, 'r') as f1:
                selected_fids_lookup = json.load(f1)
        metas = []
        vid2metaid = {}
        for vid_name in expression_datas:
            ...
            if selected_frame_file:
                selected_fids_lookup_per_exp = selected_fids_lookup[vid_name]
            for exp_id in exp_id_list:
                ...
                if selected_frame_file:
                    meta['selected_fids'] = selected_fids_lookup_per_exp['selected_frames'][exp_id]
    ...
    def __getitem__(self, index):
        ...
        if video_obj_info.get('selected_fids', None) is not None:
            data_dict['selected_fids'] = video_obj_info['selected_fids']
