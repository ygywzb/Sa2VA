...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Export MeViS qualitative cases with frames, GT masks, and prediction masks."
    )
    parser.add_argument(
        "--ours_eval_root",
        required=True,
    ...
    parser.add_argument(
        ...
        help="MeViS expression metadata path.",
    )
    parser.add_argument(
        "--mask_path",
        default="data/video_datas/mevis/valid_u/mask_dict.json",
        help="MeViS GT mask_dict path.",
    )
    parser.add_argument(
        "--image_root",
        default="data/video_datas/mevis/valid_u/JPEGImages",
        help="MeViS frame root path.",
    )
    parser.add_argument(
        "--output_dir",
        required=True,
        help="Base output directory. Results will be saved to output_dir/visualize/mevis_case_pack.",
    )
...
def resolve_eval_files(eval_root: str):
    mevis_dir = Path(eval_root) / "refvos" / "MEVIS_U"
    pred_path = mevis_dir / "results.json"
    csv_path = mevis_dir / "mevis_valu.csv"
...
def export_case(
    ...
    if ours_row is not None and baseline_row is not None:
        meta_obj["scores"] = {
            "ours": {k: ours_row[k] for k in ["J", "F", "JF"]},
            "baseline": {k: baseline_row[k] for k in ["J", "F", "JF"]},
            "delta": {
...
def main():
    ...
    output_dir = Path(args.output_dir) / "visualize" / "mevis_case_pack"
